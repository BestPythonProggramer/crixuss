...
token = 'ODY2NDMyMTYyODk0ODM5ODE4.YPSd1Q.EOTh9MHHi0goE2VRiHNNPiDTCaA'
prefix = '/'
...
administrators = ["821427615331516416"]
chat_channel = 866425326879637540
bots_channel = 866425344675807253
queue = []
...
async def on_member_join(member):
    ...
    await channel.send(f'Welcome to **Twitch Growth**, {member.mention}.\nType `/help` to get started!')
                ...
                try:
                    ...
                    if amount <= max_amount:
                        ...
                        if premium in ctx.author.roles:
                            ...
                            embed = discord.Embed(color=16777215, description=f'Adding `{amount}` followers to `{channel}`! (`{position}/{position}`)') 
                            await ctx.send(embed=embed)
